...
Vitest and Playwright files use the **`imgtor.*`** filename prefix so they sort together and stay easy to grep. `describe()` / `test()` titles stay plain (no extra `[imgtor]` prefix in output).
