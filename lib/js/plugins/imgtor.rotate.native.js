/**
 * Rotate plugin for native (Canvas 2D) adapter — same UX as Fabric rotate, no fabric.Image.rotate.
 */
(function () {
  'use strict';

  const NativeRotation = imgtor.Transformation.extend({
  ...
  imgtor.plugins['rotate'] = imgtor.Plugin.extend({
    initialize: function initNativeRotatePlugin() {
    ...
    destroy: function destroyNativeRotatePlugin() {
    ...
    rotate: function rotate(angle) {
      this.imgtor.applyTransformation(new NativeRotation({ angle: angle }));
    },
  });
})();
