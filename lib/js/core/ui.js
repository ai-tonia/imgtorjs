(function () {
  ...
  Toolbar.prototype = {
    createButtonGroup: function (options) {
      const opts = imgtor.Utils.extend(options || {}, { position: 'append' });
      const buttonGroup = document.createElement('div');
      buttonGroup.className = 'imgtor-button-group';
      if (opts.position === 'prepend' && this.element.firstChild) {
        this.element.insertBefore(buttonGroup, this.element.firstChild);
      } else {
  ...
  ButtonGroup.prototype = {
    createButton: function (options) {
      ...
      if (options.pluginId) {
        buttonElement.dataset.plugin = options.pluginId;
      }
      if (options.feature) {
        buttonElement.dataset.feature = options.feature;
      }
